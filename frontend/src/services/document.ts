...
export const documentService = {
  ...
  // 上传文本内容 (SSE 流式上传)
  uploadTextSSE: (
    data: { text_content: string; title: string; template_id: number },
    onMessage: (event: SSEEvent) => void,
    onError?: (error: Error) => void,
    onComplete?: () => void
  ) => {
    ...
    const sseClient = new SSEClient(url, data, onMessage, onError, onComplete);
    sseClient.start();
    return sseClient;
  },
  ...
  getDocuments: (params: {
    ...
    request.get<ApiResponse<PaginatedResponse<Document>>>("/documents/", {
      params,
    }),
  ...
  updateClassCode: (documentId: number, newClassCode: string) => {
    ...
    return request.put<
      ApiResponse<{ document_id: number; new_class_code: string }>
    >(`/documents/update-class-code/${documentId}`, formData, {
      headers: {
