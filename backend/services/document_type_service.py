...
class DocumentTypeService:
    ...
    async def create_document_type(
        db: AsyncSession, doc_type_data: DocumentTypeCreate
    ) -> DocumentType:
        """
        创建文档类型
        ...
        if existing:
            raise ValueError(
                f"文档类型编码 '{doc_type_data.type_code}' 在该模板下已存在"
            )
    ...
    async def get_document_types_by_template(
        db: AsyncSession, template_id: int, include_inactive: bool = False
    ) -> List[DocumentType]:
        """
        获取指定模板的所有文档类型
    ...
    async def get_document_type_by_code(
        db: AsyncSession, template_id: int, type_code: str
    ) -> Optional[DocumentType]:
        """根据编码获取文档类型"""
        result = await db.execute(
            select(DocumentType).where(
    ...
    async def update_document_type(
        db: AsyncSession, doc_type_id: int, update_data: DocumentTypeUpdate
    ) -> Optional[DocumentType]:
        """
        更新文档类型
    ...
    @staticmethod
    async def add_field(
        db: AsyncSession, field_data: DocumentTypeFieldCreate
    ) -> DocumentTypeField:
        """
        为文档类型添加字段
    ...
    async def update_field(
        db: AsyncSession, field_id: int, update_data: DocumentTypeFieldUpdate
    ...
    async def batch_update_fields(
        db: AsyncSession, doc_type_id: int, fields: List[DocumentTypeFieldSchema]
