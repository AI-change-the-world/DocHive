...
from core.tools.tool_registry import execute_tool_call
...
1. primary_keywords（主查询条件 - 必须包含）：
   - 从用户的查询语句中抽取最核心、最具体的主题词，一般为一个名词性概念。
   - 如果出现多个名词，选择“最核心的那个”，其余的降级到 context_keywords。
   - 不能将以下类型的词加入 primary：动作类（如：案例、启示、措施）、问题类（如：哪些、如何）、抽象类（如：影响、现状）、法律属性类（如：犯罪、违法、处罚）。
   - primary_keywords 数量必须为 1（极特殊情况下可为 2，但不能更多）。
   - 示例：
     - "家庭暴力犯罪的典型案例有哪些启示" → ["家庭暴力"]
     - "火山的处置措施" → ["火山"]
     - "青少年网络沉迷的危害有哪些" → ["青少年网络沉迷"]
